...
# → prompts for: name, email, GitHub username, org name / repo owner,
#   remote host, and whether to generate or reuse an SSH key
...
$ git-user switch work
  ...
  ✔ SSH alias → git-user-work-github-com (~/.ssh/id_ed25519-work)
...
work.ssh_key=~/.ssh/id_ed25519-work
...
| `ssh_key` | | Path to SSH private key used for the managed SSH alias. Generated keys default to `~/.ssh/id_ed25519-<profile>` and generated filenames are forced to end with `-<profile>`. |
...
$ git-user add personal
  ...
  Autogenerate a new SSH key for this profile using john@gmail.com? [Y/n]:
  New SSH key path [~/.ssh/id_ed25519-personal]:
  SSH key passphrase:
  Confirm SSH key passphrase:
...
$ git-user add work
  ...
  Remote host (optional) [github.com]:
  Autogenerate a new SSH key for this profile using john.smith@megacorp.com? [Y/n]: n
  Use an existing SSH key file for this profile? [y/N]: y
  ⚠  Using an existing SSH key is supported, but generating a new per-profile key is recommended.
  Existing SSH key path: ~/.ssh/id_ed25519_work
...
$ git-user add enterprise
  ...
  Autogenerate a new SSH key for this profile using john@corp.internal? [Y/n]:
  New SSH key path [~/.ssh/id_ed25519-enterprise]:
  SSH key passphrase:
  Confirm SSH key passphrase:
...
- **Multiple SSH keys** — Each profile can point to a different SSH key. `add` and `edit` can generate a new passphrase-protected Ed25519 key using the profile email as the key comment, then start `gh auth login --git-protocol ssh`. Reusing an existing key is supported, but `git-user` warns because per-profile keys are recommended.
- **Generated key names** — Autogenerated SSH private key filenames end with the profile name, such as `~/.ssh/id_ed25519-personal`, `~/.ssh/id_ed25519-work`, or `~/.ssh/id_ed25519-client-a`.
- **SSH routing** — `switch` manages per-profile SSH host aliases in `~/.ssh/config` and updates Git's global `url.*.insteadOf` rules so standard GitHub SSH remotes follow the active profile.
